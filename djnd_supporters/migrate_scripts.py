...
old_api = MauticApi(
    username="tomaz@djnd.si", password="aaaaaaaa", url="https://mautic.djnd.si/api/"
)
new_api = MauticApi()
...
for id, email in emails["emails"].items():
    print("email")
    lists = [
        segments_map[segment["id"]]
        for segment in email["lists"]
        if segment["id"] in segments_map.keys()
    ]
    if lists:
        print(lists)
        email["lists"] = lists
    ...
    assetAttachments = [
        assets_map[asset]
        for asset in email["assetAttachments"]
        if asset in segments_map.keys()
    ]
    if assetAttachments:
    ...
    if email["category"]:
        email["category"] = [
            categories_map[email["category"]]
            for cat in email["category"]
            if cat in categories_map.keys()
        ]

    response_mail, response_status = new_api.setEmail(email)
    ...
    for contact in csv_contacts:
        ...
        if contacts:
            ...
            if models.Subscriber.objects.filter(token=contact["token"]):
                subscriber = models.Subscriber(
                    token=contact["token"], username=contact["token"]
                )
            else:
                subscriber = models.Subscriber.objects.create(
                    username=contact["token"], token=contact["token"]
                )
            subscriber.save()
        ...
        if response["total"] > 0:
            for id, segment in response["lists"].items():
                ...
                if segmnet_id:
                    response, response_status = new_api.addContactToASegment(
                        segmnet_id, mautic_id
                    )
