...
class Cebelca(object):
    ...
    def add_partner(self, name, street, postal, city, **kwargs):
        data = {"name": name, "street": street, "postal": postal, "city": city}
        data.update(**kwargs)
    ...
    def set_invoice_paid(self, id_payment_method, amount):
        ...
        if self.invoice_id:
            data = {
                "date_of": datetime.now().strftime("%d.%m.%Y"),
                "amount": amount,
    ...
    def finalize_invoice(self, title=""):
        if self.invoice_id:
            data = {"id": self.invoice_id, "title": title, "doctype": 0}
    ...
    def get_pdf(self):
        if self.invoice_id:
            resp = requests.get(
                ...
                + "-pdf?id="
                + str(self.invoice_id)
                + "&format=PDF&doctitle=Ra%C4%8Dun%20%C5%A1t.&lang=si&disposition=inline&res=invoice-sent&preview=0",
                auth=requests.auth.HTTPBasicAuth(self.api_key, "x"),
            )
            if resp.status_code == 200:
    ...
    def send_mail(self, email, title, body, name):
        ...
        if pdf[0]:
            ...
            response, response_status = mautic_api.saveAsset(
                "racun", response["file"]["name"]
            )
            asset_id = response["asset"]["id"]

            response, response_status = mautic_api.getEmail(
            ...
            response, response_status = mautic_api.sendEmail(
                response_mail["email"]["id"], mautic_id, {}
            )
