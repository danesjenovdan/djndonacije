...
class ProductsList(APIView):
    def get(self, request, pk=None, format=None):
        if pk:
            ...
            articles = [
                article
                for article in Article.objects.filter(variant_of=None)
...
class Checkout(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not basket.items.all():
            return Response(
                {"status": "error", "msg": "Your basket is empty"},
...
class Pay(APIView):
    def post(self, request, format=None):
        ...
        if payment_type == "braintree":
            ...
            pay_response = payment.pay_bt_3d(
                nonce, order.basket.total, description="Shop"
            )
                ...
                return JsonResponse(
                    {
                        "msg": "failed",
                        "error": [
                            error.message for error in pay_response.errors.deep_errors
                        ],
                    },
                    status=400,
                )
            ...
            response_contact, response_status = mautic_api.getContactByEmail(
                order.email
            )
...
def checkout(request):
    if request.method == "POST":
        basket = get_basket(request)
        if not basket.items.all():
            return JsonResponse({"status": "error", "msg": "Your basket is empty"})
        basket.is_open = False
        basket.save()
        ...
        if payment_type == "braintree":
            ...
            if is_ok:
                ...
                order.save()
                url = (
                    "https://podpri.djnd.si/admin/shop/order/"
                    + str(order.id)
                    + "/change/"
                )
                ...
                return JsonResponse({"msg": "failed", "error": log})

        elif payment_type == "upn":
            reference = upn(order)
            url = (
                "https://podpri.djnd.si/admin/shop/order/" + str(order.id) + "/change/"
            )
            msg = order.name + " je nekaj naročil/-a in plačal/-a bo s položnico: \n"
            for item in basket.items.all():
                msg += " * " + str(item.quantity) + "X " + item.article.name + "\n"
            ...
            send_mail_spam(
                ...
                to_mail=to,
                file=("racun.pdf", pdf, "application/pdf"),
            )
